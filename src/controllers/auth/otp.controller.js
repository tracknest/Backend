...
import { sendOtpEmail, sendSignupEmail } from "../../email/emailService.js";
import { pendingRegistrations } from "./signup.controller.js";
import { pendingResets } from "./password.controller.js";
// ─── Verify OTP ───────────────────────────────────────────────────────────────
export const verifyOtp = async (req, res) => {
    try {
        ...
        if (pending) {
            const isValid = otpHash === pending.otpHash && pending.otpExpires > Date.now();
            if (!isValid) {
        ...
        if (pendingReset) {
            const isValid = otpHash === pendingReset.otpHash && pendingReset.otpExpires > Date.now();
            if (!isValid) {
                res.status(400).json({ message: "Invalid or expired OTP" });
                return;
            }
